...
class Critic(K.models.Model):
    ...
    def __init__(self, hidden_dims, param_dims, min_reward=None):
        ...
        self.hidden_state = K.layers.Dense(
            200,
            activation="relu",
            kernel_initializer=K.initializers.GlorotUniform(),
            kernel_regularizer=K.regularizers.L2(0.01),
        )
        self.hidden_action = K.layers.Dense(
            100,
            activation="relu",
            kernel_initializer=K.initializers.GlorotUniform(),
            kernel_regularizer=K.regularizers.L2(0.01),
        )
        self.beta_map = K.layers.Dense(
            param_dims,
            activation="softplus",
            kernel_initializer=K.initializers.GlorotUniform(),
            bias_initializer=K.initializers.random_uniform(
                minval=-0.0003, maxval=0.0003
            ),
        )
        self.gamma_map = K.layers.Dense(
            1, kernel_initializer=K.initializers.GlorotUniform()
        )
...
class Actor(K.models.Model):
    def __init__(self, action_dim, action_bound):
        ...
        self.h1 = K.layers.Dense(
            200, activation="relu", kernel_initializer=K.initializers.GlorotUniform()
        )
        self.h2 = K.layers.Dense(
            100, activation="relu", kernel_initializer=K.initializers.GlorotUniform()
        )
        self.action = K.layers.Dense(
            action_dim,
            activation="tanh",
            kernel_initializer=K.initializers.GlorotUniform(),
        )
...
class PDPGagent(object):
    def __init__(self, env, gamma, bs, bfs, lr1, lr2, delta, qs, tau):
        ...
        self.q_levels = tf.constant(
            np.append([0.0, 0.05, 0.05], np.repeat(1, 9) / 10), dtype=tf.float32
        )
        self.delta = tf.repeat(
            tf.expand_dims(self.q_levels, 0), self.BATCH_SIZE, axis=0
        )
    ...
    def linear_spline(self, arg):
        ...
        mask = tf.nn.relu(
            tf.expand_dims(alpha, axis=-1) - tf.transpose(tf.expand_dims(dl, axis=-1))
        )

        bl = tf.transpose(
            tf.expand_dims(
                tf.concat([beta[0, tf.newaxis], (beta[1:] - beta[:-1])], axis=0),
                axis=-1,
            )
        )
    ...
    def knot_value(self, arg):
        ...
        bl = tf.concat(
            [tf.expand_dims(beta[..., 0], axis=-1), (beta[..., 1:] - beta[..., :-1])],
            axis=-1,
        )
    ...
    def crps(self, arg):
        ...
        mask = tf.cast(
            tf.expand_dims(z, axis=-1) - tf.expand_dims(quantile_dl, axis=-2) > 0.0,
            tf.float32,
        )
        ...
        tilde_a = tf.clip_by_value(
            (z - gamma + tf.reduce_sum(bl * dl * mask, axis=-1))
            / (tf.reduce_sum(bl * mask, axis=-1) + 0.000001),
        ...
        crps_ = (
            ...
            + tf.math.reduce_sum(
                ...
                * (
                    (1 / 3) * (1 - dl**3)
                    - dl
                    - tf.maximum(tf.expand_dims(tilde_a, axis=-1), dl) ** 2
                    + 2 * tf.math.maximum(tf.expand_dims(tilde_a, axis=-1), dl) * dl
                ),
                axis=-1,
            )
        )
    ...
    def actor_learn(self, states):
        with tf.GradientTape() as tape:
            ...
            temp_w = (
                tf.math.log(tf.math.cumsum(self.q_levels, axis=-1) + 0.001)
                / tf.math.reduce_sum(
                    tf.math.log(tf.math.cumsum(self.q_levels, axis=-1) + 0.001), axis=-1
                )[..., tf.newaxis]
            )
            temp_quantile_dl = tf.vectorized_map(
                self.knot_value, (temp_gamma, temp_beta, self.delta)
            )
            emp_upr = tf.math.reduce_mean(
                tf.math.reduce_sum(-temp_w * temp_quantile_dl, axis=-1)
            )
    ...
    def critic_learn(self, states, actions, rewards, next_states):
        with tf.GradientTape() as tape:
            ...
            target_beta, target_gamma = self.target_critic(
                [tf.convert_to_tensor(next_states, dtype=tf.float32), next_action]
            )
            temp_alpha = tf.random.uniform(
                [self.BATCH_SIZE, self.QUANTILE_SAMPLES], minval=0.0, maxval=1.0
            )
            td_targets = tf.expand_dims(
                tf.cast(rewards, tf.float32), -1
            ) + self.GAMMA * tf.vectorized_map(
                self.linear_spline, (temp_alpha, target_gamma, target_beta, self.delta)
            )

            critic_quantile_dl = tf.vectorized_map(
                self.knot_value, (gamma, beta, self.delta)
            )
            crps = tf.math.reduce_mean(
                tf.vectorized_map(
                    self.crps, (td_targets, gamma, beta, self.delta, critic_quantile_dl)
                )
            )
    ...
    def train(self, MAX_EPISODE_NUM):
        ...
        for ep in range(MAX_EPISODE_NUM):
            ...
            while not done:
                ...
                # 행동 범위 클리핑
                action = np.clip(
                    action + self.DELTA * (self.DELTA_DECAYING_RATE) ** ep * noise,
                    -self.action_bound,
                    self.action_bound,
                )
                # 다음 상태, 보상 관측
