    ...
    def actor_learn(self, log_old_policy_pdf, states, actions, gaes):
        ...
        with tf.GradientTape() as tape:
            ...
            clipped_ratio = tf.clip_by_value(
                ratio, 1.0 - self.RATIO_CLIPPING, 1.0 + self.RATIO_CLIPPING
            )
    ...
    def train(self, max_episode_num):
        ...
        for ep in range(int(max_episode_num)):
            ...
            while not done:
                ...
                mu_old, std_old, action = self.get_policy_action(
                ...
                log_old_policy_pdf = -0.5 * (
                    action - mu_old
                ) ** 2 / var_old - 0.5 * np.log(var_old * 2 * np.pi)
                ...
                gaes, y_i = self.gae_target(
                    rewards, v_values.numpy(), next_v_value.numpy(), done
                )
                ...
                for _ in range(self.EPOCHS):
                    # 액터 신경망 업데이트
                    self.actor_learn(
                        tf.convert_to_tensor(log_old_policy_pdfs, dtype=tf.float32),
