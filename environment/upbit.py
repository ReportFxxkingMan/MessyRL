...
class UpbitEnv(gym.Env):
    def __init__(
        ...
        self._scaler_dict = {
            "volume": [data_eth["volume"].mean(), data_eth["volume"].std()],
        ...
        for col in ["volume", "candle"]:
            data_eth[col] = (
                data_eth[col] - self._scaler_dict[col][0]
            ) / self._scaler_dict[col][1]
        ...
        self.observation_space = gym.spaces.Box(
            low=-100.0, high=100.0, shape=(n_window, 2)
        )
    ...
    def reset(self) -> List:
        ...
        obs = self.data.iloc[
            (self.current_time - self.n_window) : self.current_time, :
        ][["candle", "volume"]]
